...
### /check - 代码完整度检查和等保安全审查
- 对照 Product-Spec.md 检查功能是否完整
- 对照等保三级的标准对代码规范和安全最佳实践进行严格审查
- 列出已实现和未实现的功能
- 提出补充建议
...
### 阶段 3.5：代码检查和安全审查（/check）
```
用户输入 /check
  ↓
主控检测 Product-Spec.md 是否存在（必须存在）
  ↓
调用 dev-builder 检查代码实现
  ↓
对照 Product-Spec.md 检查功能是否完整
  ↓
对照等保三级的标准对代码规范和安全最佳实践进行严格审查
...
- [ ] 需求描述清晰，无模糊表述，包含功能描述、非功能需求、用户场景、优先级等
- [ ] 功能列表完整，优先级明确，无遗漏功能、重复功能
- [ ] 非功能需求（性能、安全、可扩展性等）已考虑，无遗漏
...
- [ ] 检查 wechat_pay 配置是否完整
- [ ] 检查 app_id 是否配置正确
- [ ] 检查 mch_id 是否配置正确
- [ ] 检查 api_key 是否配置正确
- [ ] 检查 api_v3_key 是否配置正确
- [ ] 检查 serial_no 是否配置正确
- [ ] 检查 private_key_path 是否配置正确
- [ ] 检查 notify_url 是否配置正确
- [ ] 检查 cert_content 是否配置正确

## 代码质量检查要点

- [ ] 检查 安全性问题（SQL注入、XSS、敏感信息泄露等）
- [ ] 检查 代码完整性
- [ ] 检查 错误处理
- [ ] 检查 并发安全
- [ ] 检查 依赖注入是否正确
