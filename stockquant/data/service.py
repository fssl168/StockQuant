...
class DataCache:
    """In-memory + file-based K-line cache"""
    ...
    def put(self, symbol: str, timeframe: str, start: str, end: str, df: pd.DataFrame):
        if df.empty:
            return
        key = self._key(symbol, timeframe, start, end)
        self._mem_cache[key] = (df, time.time())
    ...
    def stats(self) -> Dict[str, Any]:
        return {
            ...
            "total_size_mb": round(
                sum(f.stat().st_size for f in self._cache_dir.glob("*") if f.is_file()) / (1024 * 1024), 2
            ),
        }

    def _load_from_file(self, symbol: str, timeframe: str, start: str, end: str) -> Optional[pd.DataFrame]:
...
class DataSourceResolver:
    @staticmethod
    def resolve(source: DataProvider | None = None) -> List[Tuple[str, Any]]:
        config = get_config()
        preferred = source or config.data_provider.source
...
class DataService:
    ...
    def get_kline(
        ...
        logger.debug(f"DataService.get_kline: {symbol} {timeframe} {start}/{end}")
        cached_df = self.cache.get(symbol, timeframe, start, end)
        if not cached_df.empty:
            return KlineResult(symbol, timeframe, start, end, cached_df, source="cache", cached=True)

        feeds = DataSourceResolver.resolve(provider)
        last_error = ""
        for name, feed in feeds:
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    logger.info(f"Fetched {len(df)} bars for {symbol} from {name}")
                    return KlineResult(symbol, timeframe, start, end, df, source=name, cached=False)
            except Exception as e:
                last_error = str(e)
                logger.warning(f"Provider {name} failed for {symbol}: {e}")

        return KlineResult(symbol, timeframe, start, end, pd.DataFrame(), source="none", error=last_error)

    def _fetch_from_feed(self, feed, symbol: str, timeframe: str, start: str, end: str) -> Optional[pd.DataFrame]:
    ...
    def get_multiple_klines(self, symbols: List[str], timeframe: str = "1d", start: str = "", end: str = "") -> Dict[str, KlineResult]:
    ...
    def get_health(self) -> List[Dict[str, Any]]:
        ...
        for name, feed in DataSourceResolver.resolve():
            ...
            error = ""
            try:
                test_df = self.get_kline("sh600519", timeframe="1d", start="2024-01-01", end="2024-01-05")
                ...
                healthy = False
                error = str(e)
            result.append({"provider": name, "healthy": healthy, "last_check": time.strftime("%Y-%m-%d %H:%M:%S"), "error": error})
        return result
    ...
    def upload_csv(self, content: bytes, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if missing:
                return {"success": False, "error": f"CSV missing columns: {', '.join(sorted(missing))}"}
            symbols = []
            ...
            return {"success": True, "filename": filename, "rows": len(df), "symbols": symbols, "columns": df.columns.tolist()}
        except Exception as e:
