...
def normalize_columns(
    ...
    * 默认将 *date* 列设为 DataFrame index。
    ...
    df : pd.DataFrame
        原始 DataFrame。
    provider : str
        数据源标识（``'baostock'``, ``'akshare'``, ``'csv'``, ``'yahoo'``,
        ``'generic'``）。
    date_col : str
        结果中日期列的列名，默认 ``'date'``。
    index_col : str | None
        如果指定，使用该列作为 index；None 则默认用 ``date_col``。
...
def calculate_standard_indicators(
    ...
    * ``volume_ratio`` — 当日成交量 / 5 日均量
    ...
    ----------
    df : pd.DataFrame
        已清洗的 OHLCV DataFrame。
    append : bool
        是否将指标追加到原 DataFrame；如为 ``False`` 仅返回指标 Series。

    Returns
    -------
    pd.DataFrame | dict
        追加后的 DataFrame 或仅指标字典。
    """
